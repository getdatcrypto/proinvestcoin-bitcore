...
describe('#defaultConfig', function() {
  var expectedExecPath = path.resolve(__dirname, '../../bin/proinvestcoind');
  ...
  it('will include additional services', function() {
    var config = JSON.stringify({
      ...
      services: [
        'bitcoind',
        'web',
        'proinvestcoin-insight-api',
        'proinvestcoin-insight-ui'
      ],
      servicesConfig: {
        bitcoind: {
          spawn: {
    ...
    var home = process.env.HOME;
    var info = defaultConfig({
      additionalServices: ['proinvestcoin-insight-api', 'proinvestcoin-insight-ui']
    });
    info.path.should.equal(home + '/.bitcore');
    info.config.network.should.equal('livenet');
    info.config.port.should.equal(3001);
