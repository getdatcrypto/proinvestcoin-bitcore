# proinvestcoin Service

The proinvestcoin Service is a Node.js interface to [proinvestcoin Core](https://github.com/proinvestcoin/proinvestcoin-core) for querying information about the proinvestcoin block chain. It will manage starting and stopping `proinvestcoind` or connect to several running `proinvestcoind` processes. It uses an instance of a proinvestcoin with additional indexes enabled for querying information about addresses and blocks. Results are cached for performance and there are several additional API methods added for common queries.

## Configuration

The default configuration will include a "spawn" configuration in "proinvestcoind". This defines the location of the block chain database and the location of the `proinvestcoind` daemon executable. The below configuration points to a local clone of `proinvestcoin`, and will start `proinvestcoind` automatically with your Node.js application.
...
It's also possible to connect to separately managed `proinvestcoind` processes with round-robin quering, for example:
